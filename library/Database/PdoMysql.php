...
class PdoMysql
{
    ...
    {
        $dns = sprintf("mysql:dbname=%s;host=%s;port=%s", $config['database'], $config['hostname'], $config['port'] ?: '3306');
        $this->_options = array(
            \PDO::MYSQL_ATTR_INIT_COMMAND => "set names '{$config["charset"]}'",
            \PDO::ATTR_DEFAULT_FETCH_MODE => \PDO::FETCH_ASSOC,
        );
        $this->_servers[] = array('dns' => $dns, 'username' => $config['username'], 'password' => $config['password'], $this->_options);
        $this->_lastInstance = new \PDO($dns, $config['username'], $config['password'], $this->_options);
        $this->_instances[] = $this->_lastInstance;
        $this->_name = empty($config['name']) ? ('_' . $config['database']) : ('_' . $config['name']);
    }
