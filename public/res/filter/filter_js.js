    ...
    $.fn.comboboxfilter.methods = {
            ...
            if (options.scope) {
                ...
                if ($('#' + options.scope).length>0) {
                    ...
                    node.find('.J_FilterQueryClear').unbind('click').bind('click',function() {//全部清除事件
                        ...
                        // 清空隐藏的保存值
                        options.valueId.split(',').forEach(function (value) {
        ...
        setData: function (target, options, data, targetContain) {
            ...
            $.each(data, function (i, item) {
                var listnode = $(' <li></li>');
                var idVal = item[options.idField];
                ...
                if(options.oldData.length > 0 && options.oldData.indexOf(idVal) >= 0){
                    var anyNode = target.parent().find('ul.any li a.filter-unlimit');
                    if(anyNode.hasClass('selected')){
                        anyNode.removeClass('selected');
                    }
                    if (!options.multiple) {//单选执行
                        $(targetContain).find('.selected').trigger("click.selected-tag");//触发事件清除全部选项
                    }
